...
{
    NSRect r = NSMakeRect(mWindow.frame.origin.x, mWindow.frame.origin.y, width, height);
    [mWindow setFrame:r display:NO];
}
    ...
    @autoreleasepool
    {
        ...
        while (!mClosed)
        {
            @autoreleasepool
            {
                ...
                bool sleep = !inspector && !mWindow.isKeyWindow && !runInBackground;

                [NSApp nextEventMatchingMask:NSEventMaskAny untilDate: sleep ? wait : peek
                    inMode:NSDefaultRunLoopMode dequeue:NO];

                for (;;)
                {
                    NSEvent* event = [NSApp nextEventMatchingMask:NSEventMaskAny untilDate: peek
...
{
    NS_ASSERT(cursor_->Type() < NS_COUNTOF(mCursors));
    CursorType type = cursor_->Type();
    NSCursor* cursor = mCursors[type] != nullptr ? mCursors[type] : [NSCursor arrowCursor];
...
uint32_t AppKitDisplay::GetClientWidth() const
{
    ...
    return (uint32_t)(mWindow.contentView.bounds.size.width * scale);
}
...
{
    CGFloat scale = mWindow.backingScaleFactor;
    return (uint32_t)(mWindow.contentView.bounds.size.height * scale);
}
...
void AppKitDisplay::LoadCursors()
{
    ...
    mCursors[CursorType_Arrow] = [NSCursor arrowCursor];
    mCursors[CursorType_IBeam] = [NSCursor IBeamCursor];
    mCursors[CursorType_Cross] = [NSCursor crosshairCursor];
    mCursors[CursorType_Hand] = [NSCursor pointingHandCursor];
    mCursors[CursorType_SizeNS] = [NSCursor resizeUpDownCursor];
    mCursors[CursorType_SizeWE] = [NSCursor resizeLeftRightCursor];
    mCursors[CursorType_No] = [NSCursor operationNotAllowedCursor];
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void AppKitDisplay::OnScaleChanged(CGFloat scale)
{
    mScaleChanged(this, (float)scale);
}
...
{
    mMouseWheel(this, (int)x, (int)y, (int)(delta * 100.0f));
}

////////////////////////////////////////////////////////////////////////////////////////////////////
void AppKitDisplay::OnMouseHWheel(CGFloat x, CGFloat y, CGFloat delta)
{
    mMouseHWheel(this, (int)x, (int)y, (int)(delta * 100.0f));
}
